...
class Bird(pygame.sprite.Sprite):
    def __init__(self, bg_size, landy, color, ai=False):
        ...
        if 0 <= color <= 2:
            ...
            self.images = list(map(lambda x: x.convert_alpha(), image[random.choice([0, 1, 2])]))
        elif color == 4:
            try:
                self.images = [pygame.image.load("images/birds/custom_0.png").convert_alpha(),
                          pygame.image.load("images/birds/custom_1.png").convert_alpha(),
                          pygame.image.load("images/birds/custom_2.png").convert_alpha()]
            except Exception:
                self.images = list(map(lambda x: x.convert_alpha(), image[random.choice([0, 1, 2])]))
        ...
        if not self.ai:
            self.image = pygame.transform.rotate(self.images[self.index], self.rotate)
        else:
    ...
    def move(self, delay):
        if self.ai:
            ...
            if self.rotate >= -90:
                ...
                self.image = pygame.transform.rotate(self.images[self.image_index(delay)],self.rotate)
                self.mask = pygame.mask.from_surface(self.image)
                self.center = self.rect.centerx, self.rect.centery
                self.rect = self.image.get_rect()
                self.rect.centerx, self.rect.centery = self.center
        if self.landy - self.rect.top - self.rect.height + 20 >= 0:
            self.rect.top += min(self.speedy, self.landy - self.rect.top - self.rect.height + 20)
