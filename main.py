...
class Game():
    ...
    def init_pics(self):
        # 加载背景与地面
        self.bg_black = pygame.image.load("images/bg_black.png").convert_alpha()
        self.background_list = [pygame.image.load("images/bg_day.png").convert(),
                                pygame.image.load("images/bg_night.png").convert()]
        self.land = land.Land(self.bg_size)
        
        # 游戏开始画面
        ## 游戏标题
        self.title = pygame.image.load("images/start/title.png").convert_alpha()
        self.title_rect = self.title.get_rect()
        self.title_rect.left = (self.width - self.title_rect.width) // 2
        self.title_rect.top = 80
        
        ## 开始按钮
        self.start_image = pygame.image.load("images/start/start.png").convert_alpha()
        self.start_image_rect = self.start_image.get_rect()
        self.start_image_rect.left = (self.width - self.start_image_rect.width) // 2
        self.start_image_rect.top = 240
        
        ## 排行榜按钮
        self.score_image = pygame.image.load("images/start/score.png").convert_alpha()
        self.score_image_rect = self.score_image.get_rect()
        self.score_image_rect.left = (self.width - self.score_image_rect.width) // 2
        self.score_image_rect.top = 310
        
        ## 设置按钮
        self.setting_image = pygame.image.load("images/start/setting.png").convert_alpha()
        self.setting_image_rect = self.setting_image.get_rect()
        self.setting_image_rect.left = self.width - self.setting_image_rect.width - 10
        self.setting_image_rect.top = 10
        
        # 排行画面
        ## 奖杯
        self.cups = [pygame.image.load("images/rank/gold_cup.png").convert_alpha(),
        ...
        # 设置画面
        ## 设置面板
        self.board_image = pygame.image.load("images/board.png").convert_alpha()
        self.board_rect = self.board_image.get_rect()
        self.board_rect.top = 20
        self.board_rect.left = (self.width - self.board_rect.width) // 2
        
        ## 左右箭头
        self.array_right = pygame.image.load("images/start/array.png").convert_alpha()
        ...
        ### 随机小鸟设置
        self.random_bird = [pygame.image.load("images/birds/random_0.png").convert_alpha(),
        ...
        # 游戏画面
        ## 准备图片
        self.ready_image = pygame.image.load("images/game/ready.png").convert_alpha()
        ...
        ## 点击开始图片
        self.press_start_image = pygame.image.load("images/game/tutorial.png").convert_alpha()
        self.press_start_rect = self.press_start_image.get_rect()
        self.press_start_rect.left = (self.width - self.press_start_rect.width) // 2
        self.press_start_rect.top = self.height * 0.5
        
        ## 暂停按钮
        self.pause_image = pygame.image.load("images/game/pause.png").convert_alpha()
        self.pause_image_rect = self.pause_image.get_rect()
        self.pause_image_rect.left = self.width - self.pause_image_rect.width - 10
        self.pause_image_rect.top = 10
        
        ## 继续按钮
        self.resume_image = pygame.image.load("images/game/resume.png").convert_alpha()
        self.resume_image_rect = self.resume_image.get_rect()
        self.resume_image_rect.left = self.width - self.resume_image_rect.width - 10
        self.resume_image_rect.top = 10
        
        # 分享画面
        ## 复制到剪贴板
        self.copy_image = pygame.image.load("images/share/copy.png").convert_alpha()
        self.copy_rect = self.copy_image.get_rect()
        self.copy_rect.left = (self.width - self.copy_rect.width) // 2
        self.copy_rect.top = 110
        
        ## 保存至本地
        self.save_image = pygame.image.load("images/share/save.png").convert_alpha()
        self.save_rect = self.save_image.get_rect()
        self.save_rect.left = (self.width - self.save_rect.width) // 2
        self.save_rect.top = 200
        
        ## 使用邮件分享
        self.email_image = pygame.image.load("images/share/email.png").convert_alpha()
        self.email_rect = self.email_image.get_rect()
        self.email_rect.left = (self.width - self.email_rect.width) // 2
        self.email_rect.top = 290
        
        ## 返回
        self.back_image = pygame.image.load("images/share/back.png").convert_alpha()
        ...
        # 游戏结束画面
        ## 游戏结束图片
        self.gameover_image = pygame.image.load("images/end/gameover.png").convert_alpha()
        self.gameover_image_rect = self.gameover_image.get_rect()
        self.gameover_image_rect.left = (self.width - self.gameover_image_rect.width) // 2
        self.gameover_image_rect.top = self.height * 0.12
        
        ## 得分面版
        self.score_panel = pygame.image.load("images/end/score_panel.png").convert_alpha()
        self.score_panel_rect = self.score_panel.get_rect()
        self.score_panel_rect.left = (self.width - self.score_panel_rect.width) // 2
        self.score_panel_rect.top = self.height * 0.24
        
        ## 奖牌图片
        self.white_medal = pygame.image.load("images/end/medal0.png").convert_alpha()
        self.gold_medal = pygame.image.load("images/end/medal1.png").convert_alpha()
        self.silver_medal = pygame.image.load("images/end/medal2.png").convert_alpha()
        self.brooze_medal = pygame.image.load("images/end/medal3.png").convert_alpha()
        self.medal_rect = (57, 165)
        
        ## 新纪录图片
        self.new_image = pygame.image.load("images/end/new.png").convert_alpha()
        self.new_rect = self.new_image.get_rect()
        self.new_rect.left, self.new_rect.top = 150, 139
        
        ## 再来一次图片
        self.retry_image = pygame.image.load("images/end/retry.png").convert_alpha()
        ...
        ## 分享按钮
        self.share_image = pygame.image.load("images/end/share.png").convert_alpha()
        ...
        ## 主菜单按钮
        self.menu_image = pygame.image.load("images/end/menu.png").convert_alpha()
    ...
    def init_vars(self, ai=False):
        # 读取设置
        self.bird_color, self.background_index, self.volume, self.sound_volume = setting.read_config()
        
        # 设置音量
        for i in self.sound.keys():
            self.sound[i].set_volume(self.sound_volume*self.sound_default[i]/100)
        ...
        self.R1_set = setting.Setting_line(self.screen, (64, 199), 40, 0.5, (255,0,0), 3)
        self.G1_set = setting.Setting_line(self.screen, (125, 199), 40, 0.5, (0,255,0), 3)
        self.B1_set = setting.Setting_line(self.screen, (189, 199), 40, 0.5, (0,0,255), 3)
        self.R2_set = setting.Setting_line(self.screen, (64, 249), 40, 0.5, (255,0,0), 3)
        self.G2_set = setting.Setting_line(self.screen, (125, 249), 40, 0.5, (0,255,0), 3)
        self.B2_set = setting.Setting_line(self.screen, (189, 249), 40, 0.5, (0,0,255), 3)
        self.volume_set = setting.Setting_line(self.screen, (105, 358), 110, self.volume/100, (230,100,0))
        self.sound_set = setting.Setting_line(self.screen, (105, 408), 110, self.sound_volume/100, (230,100,0))
        
        # 游戏画面
        self.bird = bird.Bird(self.bg_size, self.land.rect.top, self.bird_color, ai=ai)
        self.delay = 0
        self.paused = False
        self.pressed = False
        self.upperpipes = []
        ...
        if not ai:
            upipe, dpipe = pipe.get_pipe(self.bg_size, self.land.rect.top, self.width + 200)
        else:
            upipe, dpipe = pipe.get_pipe(self.bg_size, self.land.rect.top, self.width)
        self.upperpipes.append(upipe)
        self.lowerpipes.append(dpipe)
        self.pipe_group.add(upipe, dpipe)
        if not ai:
            upipe, dpipe = pipe.get_pipe(self.bg_size, self.land.rect.top, 1.5*self.width + 200)
        else:
            upipe, dpipe = pipe.get_pipe(self.bg_size, self.land.rect.top, 1.5*self.width)
        self.upperpipes.append(upipe)
        self.lowerpipes.append(dpipe)
        self.pipe_group.add(upipe, dpipe)
...
# ==================================键盘事件===================================
                    ...
                    if event.key == gloc.K_SPACE or event.key == gloc.K_UP:
                        # 游戏界面，小鸟存活，未暂停
                        # ----> 游戏开始/小鸟拍翅膀
                        if not self.start and not self.ranking and not self.setting and not self.paused and self.bird.alive:
                            self.pressed = True
                            # 限制小鸟高度
                    ...
                    #P键/Esc键
                        ...
                        # 游戏界面，小鸟存活，未暂停
                        # ----> 游戏暂停/开始
                        if not self.start and not self.ranking and not self.setting and self.pressed and self.bird.alive:
...
# ================================鼠标移动事件==================================
                    ...
                    if self.setting and self.mouse_down:
                        ...
                        if pygame.Rect(64, 195, 40, 11).collidepoint(pos):
                            ...
                            self.customize_bird.seperate(self.body_rgb, self.mouth_rgb)
                            self.bird = bird.Bird(self.bg_size, self.land.rect.top, self.bird_color)
                    ...
                    if event.button == 1:
                        ...
                        # 设置界面
                            ...
                            if self.setting_image_rect.collidepoint(pos):
                                self.start = True
                                self.setting = False
                                setting.write_json(self.bird_color, self.background_index, self.volume, self.sound_volume)
                                
                            # 小鸟设置
                            elif pygame.Rect(52, 105, 30, 30).collidepoint(pos):
                                self.bird_color = (self.bird_color - 1) % 5
                                self.bird = bird.Bird(self.bg_size, self.land.rect.top, self.bird_color)
                            elif pygame.Rect(202, 105, 30, 30).collidepoint(pos):
                                self.bird_color = (self.bird_color + 1) % 5
                                self.bird = bird.Bird(self.bg_size, self.land.rect.top, self.bird_color)
                                
                            # RGB设置
                            ## 身体
                            elif pygame.Rect(64, 195, 40, 11).collidepoint(pos):
                            ...
                            # 背景设置
                            elif pygame.Rect(100, 292, 30, 30).collidepoint(pos):
                                self.background_index = (self.background_index - 1) % 3
                                if self.background_index != 2:
                                    self.background = self.background_list[self.background_index]
                            elif pygame.Rect(200, 292, 30, 30).collidepoint(pos):
                                self.background_index = (self.background_index + 1) % 3
                                if self.background_index != 2:
                                    self.background = self.background_list[self.background_index]
                        ...
                        # 游戏界面，小鸟存活
                        elif self.pressed and self.bird.alive and self.pause_image_rect.collidepoint(pos):
                            self.paused = not self.paused
...
## ==================================设置======================================
                ...
                if self.bird_color in [0, 1, 2]:
                    self.screen.blit(self.bird.images[self.bird.image_index(self.delay)],
                                     (120, 100))
                elif self.bird_color == 3:
                    self.screen.blit(self.random_bird[self.bird.image_index(self.delay)],
                                     (120, 100))
                    self.screen.blit(self.random_text, ((self.width-self.random_text.get_width())//2, 150))
                elif self.bird_color == 4:
                    self.screen.blit(self.bird.images[self.bird.image_index(self.delay)],
                                     (120, 100))
                    self.screen.blit(self.custom_text, ((self.width-self.custom_text.get_width())//2, 150))
                    self.screen.blit(self.body_text, ((self.width-self.body_text.get_width())//2, 170))
                    ...
                    self.B1_set.set_point(self.body_rgb[2]/255)
                    self.B1_set.display()
                    self.screen.blit(self.mouth_text, ((self.width-self.mouth_text.get_width())//2, 220))
                ...
                self.screen.blit(self.bg_text_list[self.background_index], (150, 300))
...
## =================================排行界面====================================
                ...
                for i in range(len(self.value)):
                    ...
                    score_text = self.rank_font.render(str(self.value[i][1]), True, (0,0,0))
                    time_tran = time.strftime("%Y/%m/%d %H:%M:%S", time.localtime(self.value[i][0])).split()
                    time_text1 = self.setting_font.render(time_tran[0], True, (0,0,0))
                    time_text2 = self.setting_font.render(time_tran[1], True, (0,0,0))
                    self.screen.blit(
                            score_text,
...
## ================================游戏画面====================================
                ...
                if not self.pressed:
                    ...
                    # 绘制press开始
                    self.screen.blit(self.press_start_image, self.press_start_rect)
                else:
                    # 移动小鸟
                    if not self.paused:
                    ...
                    if self.bird.alive:
                        ...
                        if not self.paused:
                            ...
                            # 绘制继续按钮
                            self.screen.blit(self.resume_image, self.resume_image_rect)
                        
                    # 生成和删除pipe
                    if 0 < self.upperpipes[0].rect.left < 5:
                        new_upipe, new_dpipe = pipe.get_pipe(self.bg_size, self.land.rect.top)
                    ...
                    if self.bird.alive:
                        for upipe in self.upperpipes:
                            if upipe.rect.centerx <= self.bird.rect.centerx < upipe.rect.centerx + 4:
                                self.score += 1
                                self.sound['point_sound'].play()
                    
                # 检测碰撞
                    # 地面碰撞
                    if self.bird.alive and self.bird.rect.top + self.bird.rect.height >= self.land.rect.top + 20:
                        self.bird.alive = False
                        self.sound['hit_sound'].play()
                        self.sound['die_sound'].play()
                    
                    # pipe碰撞
                    if self.bird.alive and pygame.sprite.spritecollide(self.bird, self.pipe_group, False, pygame.sprite.collide_mask):
                        self.bird.alive = False
                        self.sound['hit_sound'].play()
                        self.sound['die_sound'].play()
...
## ===============================游戏结束画面==================================
                    if not self.bird.alive:
                        # 绘制gameover字样
                        self.screen.blit(self.gameover_image, self.gameover_image_rect)
                        
                        # 绘制成绩面板
                        self.screen.blit(self.score_panel, self.score_panel_rect)
                        score.show_score(self.screen, self.bg_size, self.score)
                        if not self.recorded and self.value is None:
                            self.value = score.Sql.get_score()
                        if self.value:
                            best_score = self.value[0][1]
                            score.show_best(self.screen, self.bg_size, best_score)
