...
class Customize_bird():
    ...
    def __init__(self):
        ...
        # 抠除身体和嘴部
        self.res = list(map(lambda i: cv.bitwise_and(self.images[i], self.images[i], mask=mask[i]), [0,1,2]))
    ...
    def seperate(self, body_color, mouth_color):
        for i in range(3):
            ...
            cv.imwrite("images/birds/custom_{}.png".format(i), new_bird)
...
def write_json(bird: int=3, background: int=2, music: int=50, sound: int=50) -> None:
    """
...
class Setting_line():
    ...
    def display(self):
        pygame.draw.line(self.screen, (0,0,0), self.start, self.end, self.height)
        pygame.draw.line(self.screen, self.color, self.start, self.point, self.height)
